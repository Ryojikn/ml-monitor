...
const CHANNEL_DEFS = [
  {
    ...
    fields: [
      { key: 'webhook_url', label: 'Webhook URL',           placeholder: 'https://hooks.slack.com/services/…', required: true },
...
function IntegrationCard({ def, record, onRefresh }) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (!record) {
        await api.createNotification({ type: def.type, name: def.label, config, enabled: true })
      } else {
        await api.updateNotification(record.id, { config, enabled: true })
      }
